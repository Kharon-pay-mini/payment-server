...
pub trait TransactionImpl: DbAccess {
    ...
    fn update_transaction(&self, user_id_val: &str, reference_val: &str, status: String) -> Result<usize, AppError> {
        let mut conn = self.conn().map_err(AppError::DbConnectionError)?;

        let updated_rows = diesel::update(transactions.filter(user_id.eq(user_id_val)).filter(reference.eq(reference_val)))
            .set((
